    ...
    Card {
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            if (status == null) {
                ...
                Metric("Nav phase", status.autoNavigationPhase ?: "N/A")
                Spacer(Modifier.height(4.dp))
                Text("Sensors", style = MaterialTheme.typography.titleMedium)
                Metric("Obstacle", status.sensors.obstacleDetected.yesNo())
                Metric("Proximity", status.sensors.proximityContact.yesNo())
                Metric("Contact type", status.sensors.contactType ?: "None")
                Metric("Wall side", status.sensors.wallSide ?: "None")
                Metric("Fwd clearance", status.sensors.forwardClearanceBlocked.yesNo())
